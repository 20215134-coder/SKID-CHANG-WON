...
// 게시글 편집(제목/내용/파일/참여자 등) 권한: 관리자, 팀장, 담당 파트장, 작성자 본인.
export function canEditWorkJournal(profile: Pick<AuthProfile, "id" | "role" | "isTeamLeader" | "bomCategory">, journal: JournalScope): boolean {
...
export function canDeleteWorkJournal(profile: Pick<AuthProfile, "role" | "isTeamLeader" | "bomCategory">, journal: JournalScope): boolean {
  if (profile.role === "admin" || profile.isTeamLeader) return true;
